...
static void load_commands_csv(const std::string& path, std::map<std::string,std::string>& out_map) {
    ...
    if (!in) {
        std::cerr << "[Warning] commands_csv not found: " << path << "\n";
        return;
    }
    std::string line;
    while (std::getline(in, line)) {
        ...
        // simple CSV: cmd,description (ignore commas inside quotes for simplicity)
        std::string cmd, desc;
...
bool loadConfig(const std::string& path, AppConfig& out) {
    ...
    while (std::getline(in, line)) {
        ...
        // Normalize key to lowercase for robustness
        ...
        if (key == "serial_port") {
            ...
            int v;
            if (parse_int(val, v)) out.baudrate = v;
        } else if (key == "serial_delay_ms") {
            int v;
            if (parse_int(val, v) && v >= 0) out.serial_delay_ms = v;
        } else if (key == "ollama_url") {
            out.ollama_url = val;
        } else if (key == "ollama_model") {
            out.ollama_model = val;
        } else if (key == "ollama_timeout_seconds") {
            long v;
            if (parse_long(val, v) && v >= 0) out.ollama_timeout_seconds = v;
        } else if (key == "commands_csv") {
            out.commands_csv_path = val;
            load_commands_csv(val, out.commands);
        } else {
...
bool saveConfig(const std::string& path, const AppConfig& cfg) {
    ...
    if (!cfg.commands_csv_path.empty()) {
        out << "commands_csv=" << cfg.commands_csv_path << "\n";
    } else {
        // comment a hint rather than writing a path
        out << "# commands_csv=cmds.csv\n";
    }
